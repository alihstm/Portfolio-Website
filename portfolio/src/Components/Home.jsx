      ...
      <div className="flex sm:flex-row flex-col-reverse items-start justify-between sm:w-[40%] w-full sm:h-[60%] h-[50%] sm:pl-8 px-3">
        ...
        <div className="flex flex-col items-start justify-between sm:w-[75%] w-full sm:h-[65%] h-[85%]">
          <header className="flex flex-col items-start justify-between h-[34%]">
            <h1 className="text-4xl text-old-standard-tt">
              I'm Ali Shabani
            </h1>
            <h2 className={`text-3xl font-bold text-old-standard-tt ${isDark ? 'text-white' : 'text-black'}`}>
              {displayText}
          ...
          <div className="flex flex-col items-start justify-between h-[42%]">
            <p className="text-lg text-rale-way">
              Frontend Developer & Digital Designer, Transforming Vision Into
              Interactive Beauty
            </p>

            <nav className="flex items-center gap-5">
              <AnimatedBtn name="Contact Me" className="text-rale-way" />
              <AnimatedBtn name="Download Resume" className="text-rale-way" />
            </nav>
          </div>
        </div>
      </div>

      <div className="sm:w-[50%] w-full sm:h-full h-[40%] grayPic-bg" role="img" aria-label="Profile Image"></div>
    </section>
  );
};
